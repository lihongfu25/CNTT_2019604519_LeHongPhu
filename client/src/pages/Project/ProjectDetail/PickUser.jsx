            ...
            <div className='d-flex align-items-center'>
                {selected && (
                    <div className='ratio ratio-30x30 rounded-circle overflow-hidden me-2'>
                        <img src={BASE_URL + selected.photoUrl} alt='' className='object-fit-cover' />
                    </div>
                )}
                <p className='fs-7 mb-0 color-3'>{selected ? selected.fullName || selected.email : "Chọn người phụ trách"}</p>
            </div>
            {isOpen && (
                <div className={`pick-user__select shadow position-absolute top-100 start-0 end-0 py-2 bg-color-5 rounded-6 mt-1 `}>
                    {data.length === 0 ? (
                        ...
                        data.map((item) => (
                            ...
                            >
                                <div className='ratio ratio-40x40 overflow-hidden rounded-circle'>
                                    <img src={BASE_URL + item.user.photoUrl} alt='' className='object-fit-cover' />
                                </div>
                                <div className='d-flex flex-column align-items-start ms-2'>
                                    <span className='pick-user__select__item__name fs-6 color-10'>{item.user.fullName || item.user.email}</span>
                                    {item.user.fullName && <span className='pick-user__select__item__email fs-8 color-3'>{item.user.email}</span>}
                                </div>
                            </button>
                        ))
                    )}
