...
const SelectStatus = ({
    ...
    return (
        <>
            <div
                className={`form__status d-flex align-items-center rounded-6 border position-relative ${
                    error && "is-invalid"
                }`}
                onClick={() => {
                    setIsOpen(true);
            ...
            >
                ...
                <div
                    className={`form__status__select shadow position-absolute top-100 start-0 end-0 py-2 bg-color-5 rounded-6 mt-1 ${
                        isOpen ? "" : "d-none"
                    }`}
                >
                    {data?.map((status) => (
